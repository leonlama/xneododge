...
class Orb(arcade.Sprite):
    ...
    def apply_effect(self, player):
        ...
        if self.orb_type == "speed":
            player.status_effects.apply("speed", duration=10, magnitude=0.1)
        elif self.orb_type == "multiplier":
            player.status_effects.apply("multiplier", duration=10, magnitude=1.5)
        elif self.orb_type == "cooldown":
            player.status_effects.apply("cooldown", duration=10, magnitude=0.2)
